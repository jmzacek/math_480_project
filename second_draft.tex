...
\maketitle
\section{Abstract}
MathILy is a mathematics summer camp in Philadelphia. Every summer in between regularly scheduled classes is an impromptu week-long set of classes decided by student interests. Dr. Jonah Ostroff, a UW Math professor who administrates MathILy, needs a way to quickly create an assignment of students and teachers to classes, and classes to timeslots, so that are no scheduling conflicts. We design a general linear program which creates such a schedule, and use it to build a sample schedule based on a randomized data set.
\section{Problem description}
MathILy is a summer math camp for high school students.  In the middle of the camp there is a special week called the Week of Chaos.  During the Week of Chaos a number of week-long classes are taught during some number of time slots (typically five). Each camper is scheduled for a class during each of the time slots, and each of the teachers at the camp is scheduled to teach no more than 4 of these classes.  Before the week of chaos, students are asked to fill out a survey where they rate their levels of interest in the various classes on a scale of 0-3. Teachers are asked which classes they are qualified to teach and which classes they would most like to teach. Then the camp administrators have to create a Chaos Week schedule based on student and teacher preferences. In previous years Dr. Ostroff would have to use various methods to work out a schedule by hand, reporting that it would take upwards to two hours to create a schedule for 10 students. Dr. Ostroff is concerned that as the number of teachers, students and classes increases, it will become prohibitively difficult to create an optimal schedule by hand. Our goal for this project is to create a method that will build an optimal schedule which maximizes that satisfaction of student preferences, such that students are never scheduled for the multiple classes during a given time slot, every student is assigned to a class during each time slot, and the teaching load is evenly distributed between the teachers. We will then employ our method to create an optimal schedule based on an artificial set of student and teacher preferences.
...
\indent Because there is no data yet for student preferences of 2016, we generated a set of randomized survey responses and teacher preferences for a camp that includes 5 teachers, 24 students and 15 classes, which is roughly what Dr. Ostroff expects to see for this year’s MathILy camp. We then used our method to create an optimal schedule based on this data. While our approach is built on the requirements of MathILy's chaos week, our algorithm has been generalized so that it can create optimal schedules for varying numbers of students, teachers and classes.\\
...
While this problem does not require a significant number of simplifications, some assumptions must be made about the data in order to satisfy our solutions. One assumption we must make is that each class has a teacher who is willing and able to teach it. This is a pretty benign assumption, since the camp would simply not offer a class if no one plans to teach it; still, from a mathematical this is a constraint on the sort of data our linear program is designed to handle, so it is worth mentioning. Additionally, our method doesn't allow for two or more teachers to teach a class together, as they have done in previous years. If future teachers wished to do this, wthey would either have to choose to teach a particular class beforehand and insert it into the program as a hard constraint, or else would have to make the needed adjustment ‘by hand’ after the schedule has been generated.\\
